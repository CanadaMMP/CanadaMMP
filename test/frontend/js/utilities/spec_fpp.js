...
import {
  ...
  fppData,
  proportionOfNationalPopularVoteByParty,
  seatsByParty,
...
describe("fpp.js", function(){
  ...
  describe('findWinner()', function(){
    it('finds the correct winner in each election', function(done){
      expect(controller().getEveryRecord()
        .then((docs) => docs.slice(0, 5).map((doc) => findWinner(doc))))
        .to.eventually.eql([{"party":"Liberal","familyName":"McDonald","middleName":"","firstName":"Ken","votes":23528},{"party":"Liberal","familyName":"Foote","middleName":"","firstName":"Judy M.","votes":28704},{"party":"Liberal","familyName":"Simms","middleName":"","firstName":"Scott","votes":26523},{"party":"Liberal","familyName":"Jones","middleName":"","firstName":"Yvonne","votes":8878},{"party":"Liberal","familyName":"Hutchings","middleName":"","firstName":"Gudie","votes":30889}]).notify(done);
    });
  });
  ...
  describe('proportionOfPopularVoteParty()', function(){
    it('finds the correct proportion of seats', function(done){
      expect(controller().getEveryRecord()
        .then((docs) => proportionOfNationalPopularVoteByParty (docs)))
        .to.eventually.eql({
  ...
  describe("fppData()", function(){
    ...
    it('has the right proportionOfNationalPopularVoteByParty', function(){
      expect(data.proportionOfNationalPopularVoteByParty).to.eql({
        "Independents": 0.0028204581902999794,
        "NDP-New Democratic Party": 0.19721878810796234,
        "Conservative": 0.3191111168209498,
        "Green Party": 0.03427417200201825,
  ...
  describe('ridingResultsInOrder()', function(){
    it('lists the results in winner-first order', function(){
      ...
      expect(ridingResultsInOrder(testRiding)).to.eql(expected);
    });
  });
  describe("ridingVoteWastage()", function(){
    it('calculates the vote wastage for a district', function(){
      expect(ridingVoteWastage(testRiding)).to.eql({
        winner: {
          party: 'Liberal',
          familyName: 'McDonald',
          middleName: '',
